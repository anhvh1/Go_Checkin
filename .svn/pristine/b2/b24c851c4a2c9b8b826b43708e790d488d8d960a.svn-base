...
class CheckinOut extends Component {
  ...
  setDataOnSocket = (data) => {
    ...
    if (dataCMT.HoVaTen !== "" || dataCMT.SoCMND !== "") {
      //cho vào hàng đợi
      listCheckin.push({...dataScan, imageCMTTruoc: data.anhCMND_MTBase64});
    } else {
      //điền form
  ...
  chonHangDoiCheckin = (index) => {
    ...
    const dataCMT = listCheckin[index];
    this.setState({dataCMT, imageCMTTruoc: dataCMT.imageCMTTruoc});
                    ...
                    <Col {...COL_ITEM_LAYOUT_HALF}>
                      <Row>
                        <Col {...COL_COL_ITEM_LAYOUT_RIGHT}>
                          <Item label={<span>Người cần gặp <span
                            ...
                            <TreeSelect
                              ...
                              placeholder="Chọn người cần gặp"
                              allowClear
