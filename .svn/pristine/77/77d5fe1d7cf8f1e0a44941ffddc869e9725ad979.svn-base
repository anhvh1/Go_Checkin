...
export function* loginRequest() {
  yield takeEvery('LOGIN_REQUEST', function* ({payload}) {
    ...
    if (ListConfig.data.Status > 0) {
      ...
      const pageSize = getConfigValueByKey(ListDataConfig, "Page_Size", 10);
      const defaultPassword = getConfigValueByKey(ListDataConfig, "MatKhau_MacDinh", "123456");
      const thanNhiet = getConfigValueByKey(ListDataConfig, 'Than_Nhiet', 37.5);
      const fileLimit = getConfigValueByKey(ListDataConfig, 'FileLimit', 10);
      //
      dataConfig = {
        pageSize,
        defaultPassword,
...
export function* checkAuthorization() {
  ...
  const isGuest = router.includes('guest');
  if (isGuest) {
    return;
  }
