    ...
    public class VaoRaV4DAL : IVaoRaV4DAL
    {
            ...
            try
            {
                SqlParameter[] parameters = new SqlParameter[]
                  {
                            ...
                            new SqlParameter("@ThongTinKhachID", SqlDbType.Int),
                            new SqlParameter("@LaCanBo", SqlDbType.Bit),
                            new SqlParameter("@LyDoKhac", SqlDbType.NVarChar),
                  };
                ...
                using (SqlConnection conn = new SqlConnection(SQLHelper.appConnectionStrings))
                {
                    ...
                    using (SqlTransaction trans = conn.BeginTransaction())
                    {
                        try
                        {
                            Result.Status = Utils.ConvertToInt32(SQLHelper.ExecuteScalar(trans, System.Data.CommandType.StoredProcedure, "v4_NV_ThongTinVaoRa_Insert_New", parameters), 0);
                            trans.Commit();
        ...
        public List<ThongTinVaoRaModel> TruyVetYTe_TrongDonVi(BasePagingParamsFilter p, ref int TotalRow)
        {
            ...
            try
            {
                ...
                parameters[4].Value = p.Keyword == null ? "" : p.Keyword.Trim();
                using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, System.Data.CommandType.StoredProcedure, @"v4_NV_TruyVetYTe_TrongDonVi", parameters))
                {
                    while (dr.Read())
                    {
                        var ttVaoRa = new ThongTinVaoRaModel();
        ...
        public int InsertThongTinKhach(ThongTinKhachModel ThongTinKhachModel, ref string Message)
        {
            ...
            parameters[0].Value = ThongTinKhachModel.HoVaTen;
            parameters[1].Value = ThongTinKhachModel.NgaySinh == null ? Convert.DBNull : ThongTinKhachModel.NgaySinh.Value.ToString("yyyy/MM/dd");
            parameters[2].Value = ThongTinKhachModel.GioiTinh ?? Convert.DBNull;
            parameters[3].Value = ThongTinKhachModel.HoKhau ?? Convert.DBNull;
            parameters[4].Value = ThongTinKhachModel.DienThoai ?? Convert.DBNull;
            parameters[5].Value = ThongTinKhachModel.SoCMND ?? Convert.DBNull;
            parameters[6].Value = ThongTinKhachModel.NoiCapCMND ?? Convert.DBNull;
            parameters[7].Value = ThongTinKhachModel.NgayCapCMND ?? Convert.DBNull;           
            ...
            using (SqlConnection conn = new SqlConnection(SQLHelper.appConnectionStrings))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        val = Utils.ConvertToInt32(SQLHelper.ExecuteScalar(trans, System.Data.CommandType.StoredProcedure, @"v4_ThongTinKhach_Insert", parameters), 0);
                        trans.Commit();
                    ...
                    Message = ConstantLogMessage.Alert_Insert_Success("khách");
                    return val;
                }
            }
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(SQLHelper.appConnectionStrings))
                {
                    ...
                    using (SqlTransaction trans = conn.BeginTransaction())
                    {
                        try
                        {
                            Result.Status = SQLHelper.ExecuteNonQuery(trans, System.Data.CommandType.StoredProcedure, "v4_ThongTinKhach_Update", parameters);
                            trans.Commit();
        ...
        public BaseResultModel UpdateFaceID(ThongTinKhachModel canBoInfo)
        {
            ...
            using (SqlConnection conn = new SqlConnection(SQLHelper.appConnectionStrings))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        result.Status = SQLHelper.ExecuteNonQuery(trans, System.Data.CommandType.StoredProcedure, @"v4_ThongTinKhach_UpdateFaceID", parameters);
                        trans.Commit();
                        if (result.Status < 1)
                        {
