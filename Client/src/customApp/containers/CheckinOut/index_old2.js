...
class CheckinOut extends Component {
  ...
  handleUpload = (type, info) => {
    ...
    if (type === 1) {
      getBase64(info.file.originFileObj, imageCMTTruoc => {
          ...
          api.UploadImage({image: imageCMTTruoc})
            .then(response => {
      ...
      getBase64(info.file.originFileObj, imageCMTSau => {
        ...
        api.UploadImage({image: imageCMTSau})
          .then(response => {
            if (response.data.result_code === 200) {
              this.fillData(response.data, type);
  ...
  handleUpload2 = (type, base64) => {
    if (type === 1) {
      ...
      this.setState({loadingChanDung: false, imageChanDung: base64});
      if (this.state.imageCMTTruoc === "" && this.state.imageCMTSau === "") {
        this.setState({recognitionChanDung: true});
        api.GetByChanDung({Base64File: base64})
          .then(response => {
            if (response.data.Status > 0) {
              const dataCMT = response.data.Data;
              dataCMT.NgayCapCMND = moment(dataCMT.NgayCapCMND);
  ...
  scanFile = (type) => {
    ...
    if (type === 3) {
      ...
      const base64 = this.webcamRefTruoc.current.getScreenshot({width: 1200, height: 600});
      ...
      const base64 = this.webcamRefSau.current.getScreenshot({width: 1200, height: 600});
  ...
  triggerUpload = (type) => {
    ...
    if (type === 3 && this.state.showCamera) {
      ...
      //       if (imageCMTTruoc === "" && imageCMTSau === "") {
      //         this.setState({recognitionChanDung: true});
      //         api.GetByChanDung({Base64File: imageBase64})
      //           .then(response => {
      //             if (response.data.Status > 0) {
      //               const dataCMT = response.data.Data;
      //               dataCMT.NgayCapCMND = moment(dataCMT.NgayCapCMND);
  ...
  Scan = (type) => {
    ...
    socket.onerror = (event) => {
      message.destroy();
      message.error('Không có kết nối với thiết bị quét');
      if (type === 1) {
        this.setState({loadingTruoc: false})
      } else if (type === 2) {
        this.setState({loadingSau: false})
      }
      return Data;
    };
        ...
        socketData.onmessage = (event) => {
          ...
          reader.onloadend = () => {
            ...
            if (type === 1) {
              ...
              api.UploadImage({image: base64data})
                .then(response => {
                  if (response.data.result_code === 200) {
                    this.fillData(response.data, type);
                    this.setState({
    ...
    if (navigator.getUserMedia) {
      navigator.mediaDevices.enumerateDevices().then(devices => {
        ...
        if (videoInput.length) {
          ...
          message.destroy();
          message.warn('Không có kết nối camera');
        }
      }).catch(error => {
        message.error(error.toString());
      });
  ...
  changeCamera = () => {
    ...
    indexCamera = indexCamera === 1 ? 0 : 1;
    this.setState({indexCamera});
  };

  captureCamera = () => {
    const {listCapture} = this.state;
    const imageSrc = this.webcamRef.current.getScreenshot();
    listCapture.push(imageSrc);
    this.setState({listCapture});
  };
  ...
  render() {
    ...
    const propsTruoc = {
      ...props,
      beforeUpload: file => this.beforeUpload(file, 1, this.handleUpload2),
      disabled: loadingTruoc || loadingSau || loadingChanDung,
      id: 'uploadTruoc'
    };
    const propsSau = {
      ...props,
      beforeUpload: file => this.beforeUpload(file, 2, this.handleUpload2),
      disabled: loadingTruoc || loadingSau || loadingChanDung,
      id: 'uploadSau'
    };
    const propsChanDung = {
      ...props,
      beforeUpload: file => this.beforeUpload(file, 3, this.handleUpload2),
    ...
    return (
      <LayoutWrapper>
        ...
        <PageAction>
          {roleBaoCao.view ? <Link to="bao-cao">
            <Button type={'primary'} style={{height: 30}} onClick={() => this.props.changeCurrent(['bao-cao'])}>Xem danh
              sách</Button>
          </Link> : ""}
        </PageAction>
        <Box style={{minHeight: 270, marginBottom: 5, border: 'none'}}>
          <div className={'upload-container'}>
            <div className={'upload-div'}>
              <Dragger {...propsTruoc}>
                {/*{loadingTruoc && imageCMTTruoc === "" ? <Icon type={'loading'}/> : imageCMTTruoc !== "" ?*/}
                {/*  <img src={imageCMTTruoc} alt="avatar" style={{maxHeight: 150}} id={"imgTruoc"}/> : uploadCMTTruoc}*/}
                {loadingTruoc ? <Icon type={'loading'}/> : imageCMTTruoc !== "" ?
                  <img src={imageCMTTruoc} alt="avatar" style={{maxHeight: 150}}
                       id={"imgTruoc"}/> : ""}
                <div
                  style={loadingTruoc ? {display: 'none'} : imageCMTTruoc !== "" ? {display: 'none'} : {}}>{cameraContentScan}
                </div>
              </Dragger>
              <div className={'button-scan'}>
                <Button icon={'upload'}
                        style={{backgroundColor: '#ccc', color: 'black', fontWeight: 'normal !important'}}
                        onClick={() => this.triggerUpload(1)}
                        disabled={loadingTruoc || loadingSau || loadingChanDung}>Chọn file</Button>
                <Button icon={'search'} disabled={loadingTruoc || loadingSau || loadingChanDung}
            ...
            <div className={'upload-div'}>
              <Dragger {...propsSau}>
                {/*{loadingSau && imageCMTSau === "" ? <Icon type={'loading'}/> : imageCMTSau !== "" ?*/}
                {/*  <img src={imageCMTSau} alt="avatar" style={{maxHeight: 150}}/> : uploadCMTSau}*/}
                {loadingSau ? <Icon type={'loading'}/> : imageCMTSau !== "" ?
                  <img src={imageCMTSau} alt="avatar"
                       style={{maxHeight: 150}}/> : ""}
                <div
                  style={loadingSau ? {display: 'none'} : imageCMTSau !== "" ? {display: 'none'} : {}}>{cameraContentScan2}
                </div>
              </Dragger>
              <div className={'button-scan'}>
                <Button style={{backgroundColor: '#ccc', color: 'black'}} icon={'upload'}
                        onClick={() => this.triggerUpload(2)}
                        disabled={loadingTruoc || loadingSau || loadingChanDung}>Chọn file</Button>
                <Button icon={'search'} disabled={loadingTruoc || loadingSau || loadingChanDung}
                        onClick={() => this.scanFile(2)}
                        style={{marginLeft: 10, backgroundColor: '#ccc', color: 'black'}}>
                  Quét file
                </Button>
              </div>
            </div>
            <div className={'upload-div'}>
              <Dragger {...propsChanDung}>
                <div>
                  {loadingChanDung && imageChanDung === "" ?
                    <Icon type={'loading'}/> : imageChanDung !== "" && !showCamera ?
                      <img src={imageChanDung} alt="avatar"
                           style={{maxHeight: 150}}/> : showCamera ? cameraContentChanDung : uploadChanDung}
                </div>
              </Dragger>
              <div className={'button-scan'}>
                <Button style={{backgroundColor: '#ccc', color: 'black'}} icon={showCamera ? 'picture' : 'upload'}
                        onClick={() => this.triggerUpload(3)}
                        disabled={loadingTruoc || loadingSau || loadingChanDung}>{showCamera ? "Chụp ảnh" : "Chọn file"}</Button>
                <Button icon={'camera'} disabled={loadingTruoc || loadingSau || loadingChanDung}
                        onClick={() => this.scanFile(3)}
                        style={{marginLeft: 10, backgroundColor: '#ccc', color: 'black'}}>
        ...
        <Box title={'THÔNG TIN VÀO - RA'}
             style={{paddingTop: 5, marginTop: 5, marginBottom: 5, paddingBottom: 5, border: '1px solid #ccc'}}>
          <Form>
            <Row>
              <Col {...COL_ITEM_LAYOUT_HALF}>
                <Item label={"Giờ vào"} {...ITEM_LAYOUT_HALF} className={'datepicker'}>
                  ...
                  <Input placeholder={""} style={{width: '30%', maxWidth: 100}}
                         value={isCheckIn || isCheckOut || checkCheckOut ? valueGioVaoHS : ""}
                         disabled
                  ...
                  <Input placeholder={""} style={{width: 'cacl(70% - 20px)', marginLeft: 20, maxWidth: 180}}
                         value={isCheckIn || isCheckOut || checkCheckOut ? valueGioVaoDMY : ""}
                         disabled
                ...
                <Row>
                  <Col {...COL_COL_ITEM_LAYOUT_RIGHT}>
                    <Item label={"Gặp cán bộ"} {...ITEM_LAYOUT_HALF}>
                      <Select style={{width: '100%'}} placeholder={'Chọn cán bộ cần gặp'} optionFilterProp={'label'}
                    ...
                    <Item label={"Mã thẻ"} {...ITEM_LAYOUT_HALF}>
                      <PopoverCustom content={this.contentPopover(dataPopover, 1)} visible={visiblePopoverMaThe}
                        ...
                        <Input.Search style={{width: '100%'}} onSearch={value => this.blurMaThe(value, 1)}
                                      maxLength={20} value={dataCMT.MaThe}
                ...
                <Row>
                  <Col {...COL_COL_ITEM_LAYOUT_RIGHT}>
                    <Item label={"Ngày cấp"} {...ITEM_LAYOUT_HALF}>
                      ...
                      <DatePicker onChange={value => this.changeValueCMT("NgayCapCMND", value)} format={"DD/MM/YYYY"}
                                  style={{width: '100%'}} placeholder={""} value={dataCMT.NgayCapCMND}/>
                    </Item>
                  </Col>
                </Row>
              </Col>
                ...
                <Row>
                  <Col {...COL_COL_ITEM_LAYOUT_RIGHT}>
                    <Item label={"Nơi cấp"} {...ITEM_LAYOUT_HALF}>
                      <Input style={{width: '100%'}} value={dataCMT.NoiCapCMND}
                             onChange={value => this.changeValueCMT("NoiCapCMND", value.target.value)}/>
                    </Item>
                  </Col>
                </Row>
              </Col>
            ...
            <Row>
              <Col {...COL_ITEM_LAYOUT_HALF}>
                <Item label={"Ngày sinh"} {...ITEM_LAYOUT_HALF}>
                  ...
                  <DatePicker onChange={value => this.changeValueCMT("NgaySinh", value)} format={"DD/MM/YYYY"}
                              style={{width: '100%'}} placeholder={""} value={dataCMT.NgaySinh}/>
                </Item>
              </Col>
              <Col {...COL_ITEM_LAYOUT_HALF}>
                <Row>
