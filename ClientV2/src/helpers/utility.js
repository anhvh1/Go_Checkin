...
export function formatMissionData(DataApi) {
  const DanhSachTrangThai = DataApi.DanhSachTrangThai
    ? DataApi.DanhSachTrangThai
    : [];
    ...
    DanhSachNhiemVu.forEach((NhiemVu) => {
      if (NhiemVu.TrangThaiID === TrangThai.ID) {
        items.push({
          ...
          date: NhiemVu.NgayHetHan
            ? moment(NhiemVu.NgayHetHan).format("DD/MM/YYYY")
            : moment().format("DD/MM/YYYY"),
          total_comment: NhiemVu.SoBinhLuan,
          total_file: NhiemVu.SoFileDinhKem,
          work_progress: NhiemVu.SoCongViec
            ? `${NhiemVu.SoCongViecDaXuLy}/${NhiemVu.SoCongViec}`
            : "",
          member_list: NhiemVu.DanhSachCanBo.length
            ? NhiemVu.DanhSachCanBo.map((item) => ({
                ID: item.ID,
                Ten: item.Ten,
              }))
            : NhiemVu.DanhSachCoQuan.map((item) => ({
                ID: item.ID,
                Ten: item.Ten,
              })),
        });
    ...
    if (ds[i].children) {
      ...
      for (let j = 0; j < ds[i].children.length; j++) {
        if (ds[i].children[j].children) {
          ds[i].children[j].children = ds[i].children[j].children.filter(
            (item) => item.CQCoHieuLuc
          );
        }
      }
    }
  ...
  return Data.map((value, index) => {
    ...
    let children =
      value.Children && value.Children.length > 0
        ? renderChildrenCoQuanCanBo(value.Children, index, key)
        : null;
    return {
      ...value,
      title: title,
...
export function getOptionSidebar() {
  ...
  if (
    optionsSidebar[0].children.length &&
    optionsSidebar[0].children[1].key === "nhiem-vu"
  ) {
    optionsSidebar[0].children.splice(2, 0, {
      key: "nhiem-vu/cong-viec",
      label: "Công việc",
    });
  }
