...
class GuestCheckin extends Component {
  ...
  setSessionCode = () => {
    ...
    this.setState({
      sessionCode: `${random()} ${random()} ${random()} ${random()} ${random()} ${random()}`,
    });
  };
  ...
  fillData = (data, AnhCMND_MTBase64) => {
    const { dataCMT, sessionCode } = this.state;
    ...
    // }
    this.setState({ dataCMT, validCheckin: true }, () => {
      this.openModalCheckin();
    });
  };
    ...
    return (
      <Styled className={"index"}>
        ...
        <div className={"camera-container"} style={{ ...resolutionOfDiv }}>
          ...
          {recognition ? (
            <div className={"loading"}>Đang quét tài liệu</div>
          ) : (
            ""
          )}
        </div>
        <div className={"action"}>
          <CameraOutlined type={"camera"} onClick={this.captureCamera} />
          {isMobile ? "" : <SyncOutlined onClick={this.switchCamera} />}
          {validCheckin && !visibleModalCheckin ? (
            <Button type={"primary"} onClick={this.openModalCheckin}>
