...
namespace Com.Gosol.INOUT.API.Controllers.QuanTriHeThong
{
    ...
    [Route("api/v1/VaoRa")]
    [ApiController]
    public class VaoRaController : BaseApiController
    {
        private IVaoRaBUS _VaoRaBUS;
        private ISystemConfigBUS _SystemConfigBUS;
        public IOptions<AppSettings> settings;
        public VaoRaController(IOptions<AppSettings> Settings,IVaoRaBUS VaoRaBUS, ISystemConfigBUS SystemConfigBUS, ILogHelper _LogHelper, ILogger<VaoRaController> logger) : base(_LogHelper, logger)
        {
        ...
        [CustomAuthAttribute(ChucNangEnum.CheckIn_CheckOut, AccessLevel.Read)]
        public IActionResult Get_By_AnhChanDung(FileDinhKemModel fileDinhKem)
        {
            try
            {
                var DonViSuDungID = Utils.ConvertToInt32(User.Claims.FirstOrDefault(x => x.Type == ("CoQuanSuDungPhanMem")).Value, 0);
        ...
        //[CustomAuthAttribute(ChucNangEnum.CheckIn_CheckOut, AccessLevel.Read)]
        public IActionResult DanhSachDoiTuongGap()
        {
            try
            {
                var CoQuanID = Utils.ConvertToInt32(User.Claims.FirstOrDefault(x => x.Type == ("CoQuanID")).Value, 0);
                return CreateActionResult("Danh sách đối tượng gặp của cơ quan " + CoQuanID, EnumLogType.GetByName, () =>
                {
                    var clsCommon = new Commons();
                    var Result = _VaoRaBUS.DanhSachDoiTuongGap(CoQuanID);
