...
{
    public interface IDanhMucCoQuanDonViV4DAL
    {
    ...
    public class DanhMucCoQuanDonViV4DAL : IDanhMucCoQuanDonViV4DAL
    {
        ...
        public BaseResultModel ThemCanBoVaoDonViSuDungPhanMen(ThongTinDangKyModel DangKyInfo)
        {
            ...
            try
            {
                if (DangKyInfo.DanhSachCanBo != null && DangKyInfo.DanhSachCanBo.Count > 0)
                {
                    ...
                    {
                        //không bắt buộc nhập email. ko có tự sinh
                        if (item.Email == null || string.IsNullOrEmpty(item.Email))
                        {
                            if (item.TenCanBo != null && item.TenCanBo.Length > 0)
                            {
                                var ten = Utils.NonUnicode(item.TenCanBo.Trim());
                                ...
                                var tenHopLe = tenGoiY.ToLower();
                                item.TenTaiKhoan = tenHopLe;
                            }
                        }
                        else
                        {
                ...
                if (DangKyInfo.DanhSachCanBo != null && DangKyInfo.DanhSachCanBo.Count > 0)
                {
                    foreach (var item in DangKyInfo.DanhSachCanBo)
                    {
                        ...
                        var isCanBo = new HeThongCanBoV2DAL().ThemMoiCanBoChoPhongBan_v4(item, isThemMoiPhongBan);
                        if (isCanBo.Status < 1)
                        {
                            Result.Message = ConstantLogMessage.API_Error;
                            Result.Status = -1;
